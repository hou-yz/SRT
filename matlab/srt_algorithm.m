...
for j=(1:J_range) %遍历用户j
    ...
    while C_tmp(j,T_range+1)<C_max(j) %若不满足qos条件，增加一条边
        ...
        if C_tmp(j,T_range+1)+r(0+1,j,t0)*delta_t<C_max(j)%未溢出
            ...
            rho(0+1,j,t0)=(C_max(j)-C_tmp(j,T_range+1))/(r(0+1,j,t0)*delta_t);
            C_tmp(j,t0:T_range+1)=C_tmp(j,t0:T_range+1)+r(0+1,j,t0)*delta_t*rho(0+1,j,t0);
        end
...
while sum(t_OOC)~=0 %超出基站能力范围
    %更新
    for t=(1:T_range)
        t_free(t)=(sum(rho(0+1,:,t)>0)<=N);
        t_OOC(t)=(sum(rho(0+1,:,t)>0)>N);
    end
    r_i_free=reshape(r(1,:,:).*(1-rho(0+1,:,:)),J_range,T_range).*(ones(J_range,1)*t_free);% 空闲时隙中 可用 速率
    r_i_OOC=reshape(r(1,:,:),J_range,T_range).*(ones(J_range,1)*t_OOC);r_i_OOC(rho(0+1,:,:)==0)=0;r_i_OOC(r_i_OOC==0)=NaN;% 超出能力时隙对应速率
    ...
    S(S_index,:)=[];
    C_tmp(j,t0:T_range+1)=C_tmp(j,t0:T_range+1)-r(0+1,j,t0)*rho(0+1,j,t0)*delta_t;
    rho(0+1,j,t0)=0;
    %fprintf('去掉一条边：[0 %d %d]\n',j,t0)
    %更新
    for t=(1:T_range)
    ...
    r_i_free=reshape(r(1,:,:).*(1-rho(0+1,:,:)),J_range,T_range).*(ones(J_range,1)*t_free);
    ...
    while C_tmp(j,T_range+1)<C_max(j)%去掉该边后，寻找一组次优解满足QoS限制
        ...
        if C_tmp(j,T_range+1)+r_ijt0*delta_t<C_max(j)%未溢出
            rho(0+1,j,t0)=1;% 和之前delta取值无关，现在全都更新为1
            C_tmp(j,t0:T_range+1)=C_tmp(j,t0:T_range+1)+r_ijt0*delta_t*rho(0+1,j,t0);
        else%溢出
            rho(0+1,j,t0)=rho(0+1,j,t0)+(C_max(j)-C_tmp(j,T_range+1))/(r_ijt0*delta_t);
            C_tmp(j,t0:T_range+1)=C_tmp(j,t0:T_range+1)+r_ijt0*delta_t*rho(0+1,j,t0);
        end
...
rho_2=reshape(rho(0+1,:,:),J_range,T_range);
eta_2=reshape(eta(0+1,:,:),J_range,T_range);
C_unsatisfied=C_qos-C_max;

%第三部分
r0_index=find(rho(0+1,:,:)>0);
[~,jj,tt]=ind2sub(size(rho(0+1,:,:)),r0_index);
for j=(1:J_range)%for all user j
    ...
    for t2=(1:T_range)
        for j_prime_R=(1:J_range)
            ...
            if j_prime_R~=j && sum(rho(:,j_prime_R,t2)>0)+sum(rho(j_prime_R+1,:,t2)>0)-2*(rho(j_prime_R+1,j,t2)>0)==0 && sum(rho(:,j,t2)>0)-2*(rho(j_prime_R+1,j,t2)>0)+sum(rho(j+1,:,t2)>0)==0 && sum(sum(rho(:,:,t2)>0))-(rho(j_prime_R+1,j,t2)>0)<N && rho(j_prime_R+1,j,t2)<1 && r2>r0_min % j' and j and sys free
                for t1=(1:T_range)
                    if t1~=t2 && sum(rho(:,j_prime_R,t1)>0)-2*(rho(0+1,j_prime_R,t1)>0)+sum(rho(j_prime_R+1,:,t1)>0)==0 && sum(sum(rho(:,:,t1)>0))-(rho(0+1,j_prime_R,t1)>0)<N && rho(0+1,j_prime_R,t1)<1 % j' free and sys availible
    ...
    while ~isempty(R)
        r0_index=find(rho(0+1,:,:));% 更新可选r0对应数据
        [~,jj,tt]=ind2sub(size(rho(0+1,:,:)),r0_index);
        r0_j=r(0+1,j,tt(jj==j));
        ...
        for i_0=(1:length(tt(jj==j)))
            ...
            for i_12=(1:size(R,1))
                ...
                P1=(2^(r0_this*rho(0+1,j,t0)/BW)-1)*sigma2/(beta1*h0^2);
                P2=(2^(r0_this*rho(0+1,j,t0)/BW)-1)*sigma2/(beta2*h0^2);
                delta_rho_1=r0_this*rho(0+1,j,t0)/r(0+1,j_prime,t1);
                delta_rho_2=r0_this*rho(0+1,j,t0)/r(j_prime+1,j,t2);
                ii=(i_0-1)*size(R,1)+i_12;
                if rho(0+1,j_prime,t1)+delta_rho_1<1 && rho(j_prime+1,j,t2)+delta_rho_2<1 && sum(rho(:,j_prime,t1)>0)-2*(rho(0+1,j_prime,t1)>0)+sum(rho(j_prime+1,:,t1)>0)==0 && sum(rho(:,j_prime,t2)>0)+sum(rho(j_prime+1,:,t2)>0)-2*(rho(j_prime+1,j,t2)>0)==0 && sum(rho(j+1,:,t2)>0)+sum(rho(:,j,t2)>0)-2*(rho(j_prime+1,j,t2)>0)==0 && sum(sum(rho(:,:,t2)>0))-(rho(j_prime+1,j,t2)>0)<N && sum(sum(rho(:,:,t1)>0))-(rho(0+1,j_prime,t1)>0)<N
        ...
        if m>0
            ...
            %fprintf('去掉一条边：[0 %d %d]\n',j,t0)
            %加入edge2边
            rho(j_prime+1,j,t2)=rho(j_prime+1,j,t2)+delta_rho_2;
            ...
            %加入edge1边
            rho(0+1,j_prime,t1)=rho(0+1,j_prime,t1)+delta_rho_1;
