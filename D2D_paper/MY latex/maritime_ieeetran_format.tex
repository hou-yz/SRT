\documentclass[conference]{IEEEtran}
 ...
 %With the aid of large-scale CSI, we can have maximum benefit from the mobility of ships by proper user shceduling. 
 ...
 %With delay-tolerant service assumption and large-scale CSI, we address the long-term user requirement problem and the CSI obtaining problem based on the characteristic of the maritime communication system. 
 
 %On that basis, we enlarge the optimization subspace by introducing the time dimension. Together with the transmitter dimension introduced by IoV since IoV's superiority in energy consumption, spectral efficiency and BS coverage, our maritime user scheduling scheme explore the 3-dimensional optimization subspace for energy consumption improvement rather than the 1-dimensional (receiver dimension only) optimization subspace in the traditional ship-to-shore method. 
 
 % proposed method
 
 In this paper, with the help of large-scale CSI, we formulate an optimization problem for user scheduling in maritime ship-to-ship/shore communication systems, aiming to minimize the energy consumption. % while providing users with delay-tolerant data distribution services. 
 Unfortunately, ship-to-ship relay transmission introduces more transmitters (BS/relays), which brings more difficulties in user scheduling and make the problem incomputable. % (see Appendix A)
 ...
 %\end{align}
 % and the transmission use ${\eta _{i,j,t}}$ of the transmitter's max transmission power. 
...
The algorithm in step-2 is carried out as follows.
 ...
 \FOR{all user $j$}
  \WHILE{${\left. {{V_{j,t}}} \right|_{t = T}} < 2{V^{QoS}}$}
 ...
 \WHILE{${\left. {{V_{j,t}}} \right|_{t = T}} \ge {V^{QoS}}$}
 ...
 \begin{subnumcases}
 {}%\begin{align}
 {V_{j',{t_2} - 1}} \ge r_0^{\min }\Delta \tau ,{\textit{if}}\;{t_1} > {t_2},\\
...
Of all the following simulations in Figure 2 - 4, %the `round-robin' method refers to the ship-to-shore only method based on zero CSI. 
...
They are brought into the following simulations to examine the feasibility of our large-scale CSI method. 
 ...
 This 5\% error in ship-to-ship/shore systems shows that the large-scale CSI replacement in (2d) is quite acceptable. %If we are less strict with the approximation, the ${h_0}$ constant estimation may be acceptable since the estimation in (12) shares similarity in trend and shape with the actual channel. 
 ...
 %\Figure[htb]{Cqos.png}
 %{Average energy consumption per user $E_{avg}$ versus the QoS constraint ${V_{QoS}}$.\label{fig3}}
...
% The inflection point is larger in ship-to-ship/shore system since the transmission speed gain from lower ship relay transmission power is larger than higher BS transmission power, thus similarly the relay transmission can be done more quickly and in better CSI conditions. 
 ...
 %As the SNR decreases, the channel capacity worsens and the greedy reference method end up with more low-speed links. Thus we can see an improvement of over 60\% in energy consumption from the proposed ship-to-ship/shore method when ${\sigma ^2 \le{10^{ - 10}}}$.
 ...
 % from optimization subspace with a higher dimension. 
