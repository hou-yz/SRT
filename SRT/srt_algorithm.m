...
P1_tmp=linspace(0,P_i_max,101);P2_tmp=linspace(0,P_j_max,11);
...
for j=(1:J_range) %遍历用户j
    [~,beta_j_sorted_t_index]=sort(r(1,j,bt(j):et(j)),'descend');
    ijt_t_pointer=1;
    ...
    for j_prime_R=(1:J_range)
        ...
        for t2=t2s
            ...
            if j_prime_R~=j && sum(eta(:,j_prime_R,t2)>0)+sum(eta(j_prime_R+1,:,t2)>0)-2*(eta(j_prime_R+1,j,t2)>0)==0 && sum(eta(:,j,t2)>0)-2*(eta(j_prime_R+1,j,t2)>0)+sum(eta(j+1,:,t2)>0)==0 && sum(sum(eta(:,:,t2)>0))-(eta(j_prime_R+1,j,t2)>0)<N && eta(j_prime_R+1,j,t2)<1% j' and j and sys free
                ...
                t1s=(bt(j):et(j));
                t1s=t1s(r(0+1,j_prime_R,t1s)>r0_max);
                for t1=t1s
...
%         r0_j=r(0+1,j,tt0(jj0==j));
        ...
        for i_0=(1:length(tt0(jj0==j)))
            ...
            if t_step>=1
                ...
                gamma=beta(0+1,j,t0)*P(0+1,j,t0)*eta(0+1,j,t0)/sigma2;
                F_int=integral(@(x) F_x(x,1/gamma),1,inf,'ArrayValued',true);
                r0=BW*log2(exp(1)).*exp(1/gamma).*F_int;
            end
...
%             r0_prime=r(0+1,j,t0)*eta(0+1,j,t0);
            for i_12=(1:size(R,1))
                ...
                beta1=beta(0+1,j_prime,t1);beta2=beta(j_prime+1,j,t2);
                    ...
                    if t_step>=1
                        r1_tmp=sum(BW*log(1+P1_tmp.*ones(1,1,t_step).*(beta1.*ones(1,length(P1_tmp),t_step)).*(abs(h0(0+1,j_prime,(t1-1)*t_step+1:t1*t_step).*ones(1,length(P1_tmp),t_step))).^2/sigma2)/log(2),3)/t_step;
                        r2_tmp=sum(BW*log(1+P2_tmp.*ones(1,1,t_step).*(beta2.*ones(1,length(P2_tmp),t_step)).*(abs(h0(j_prime+1,j,(t2-1)*t_step+1:t2*t_step).*ones(1,length(P2_tmp),t_step))).^2/sigma2)/log(2),3)/t_step;
                    else
                        gamma1=beta1.*P1_tmp/sigma2;
                        F_int=integral(@(x) F_x(x,1./gamma1),1,inf,'ArrayValued',true);
                        r1_tmp=BW*log2(exp(1)).*exp(1./gamma1).*F_int;
                        r1_tmp(isnan(r1_tmp))=0;
                        gamma2=beta2.*P2_tmp/sigma2;
                        F_int=integral(@(x) F_x(x,1./gamma2),1,inf,'ArrayValued',true);
                        r2_tmp=BW*log2(exp(1)).*exp(1./gamma2).*F_int;
                        r2_tmp(isnan(r2_tmp))=0;
                    end
                    ...
                    P2=P2_tmp(P2_index);
%                     [~,P1_index_g]=min(abs(r1_tmp_g-r0));
%                     [~,P2_index_g]=min(abs(r2_tmp_g-r0));
%                 if abs(P1_index_g-P1_index)>20 ||abs( P2_index_g-P2_index)>2
%                     [0,j,t0]
%                 end
